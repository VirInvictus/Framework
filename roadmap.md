...
- [ ] **HIGH (NEW): PDF metadata stack out-of-bounds (fw-document-pdf.c:1258-1262): metadata_lookup treats fz_lookup_metadata's return as a buffer length; MuPDF documents it as the UNTRUNCATED size, so a PDF with Title/Author/Keywords > 1023 bytes overruns the 1024-byte stack buffer via Document Properties.** One-line clamp of n to sizeof buf - 1.
- [ ] **HIGH (CONFIRMED): DjVu ddjvu_document_get_* calls run unlocked against the shared context, racing render workers** (fw-document-djvu.c:409,496,521,549,565,597,640). Take render_lock around all of them; add the lock-discipline header paragraph.
- [ ] **HIGH (CONFIRMED): MOBI INDX second-pass infinite loop on read_var_len length 0** (fw-mobi-parser.c:604-610; the CNCX loop has the guard, this pass does not).
- [ ] MED: Memory-safety family: cache rendering-bool reset defeats the eviction guard (fw-cache.c:681 vs 796-808, replace with in-flight count); ExtractCtx raw window* across the portal async (fw-window.c:886-897; the "weak: not refed" comment manufactures a lifetime guarantee that does not exist); MOBI get_resources extra ref never dropped (transfer-none contract); EPUB/FB2 archive entries uncapped; MOBI inline <style> breakout missing the EPUB "</style" guard; MOBI hdr_length 32-bit overflow; CBR non-volatile fz_try locals.
- [ ] MED: Dead-code sweep: fw_cache_get_page + fw_cache_page_ready (zero callers); webview PositionRequest machinery (~55 lines, duplicated by the live user-script path); MOBI eager GdkTexture decode (never read); fw_document_close wrapper + reflow close chain (both dead lifecycles); cmp_uint/cmp_guint32 duplicates.
- [ ] MED: Docs residue: desktop MimeType missing text/plain (TXT claimed by spec + shipped in the file dialog); spec drag-and-drop still "TBD" (decided in code v0.10); two sections numbered 2.4 + state-sketch keys zoom/scroll_y (code: zoom_level/scroll_position); fallback-to-MuPDF claim false for TXT/MD under fixed-pages mode; CLAUDE.md stale bits ("three stress tests" → seven, 6/7 ASan posture, Phase 6 open → shipped, get_resources "EPUB-only" → EPUB+MOBI, reflow gate missing Markdown, scope list missing TXT, Calibre-Library-as-default-corpus); roadmap corpus-root box + screenshots box + fw-window line count; README HiDPI shipped-claim + Phase 13.1 parenthetical + "5-figure" textbook; spec search debounce 300ms → GtkSearchEntry 150ms default; tests/README FW_TEST_CORPUS_ROOT is runtime, not configure-time.
- [ ] MED: Em-dash ban: 348 rendered violations (roadmap 152 bold-label separators, README 68 including mdash entities, CLAUDE.md 47, foliate-rewrite 47, spec 29); plus slop kills: "Best-in-Class UX" Phase-14 tagline, "Leveraging", "Seamless Auto-Reload", "marquee delivery", "design north star", "utilizing", "seamless" duplicated across README/roadmap, "core performance differentiator" duplicated spec/CLAUDE.
