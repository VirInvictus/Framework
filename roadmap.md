...
- [ ] **Publisher CSS + cross-chapter links** (Phase 17.x): a `framework-css:` scheme to honour publisher stylesheets; rewrite cross-chapter hrefs to in-doc fragments. Includes **EPUB font de-obfuscation**: once fonts are actually served, de-obfuscate embedded fonts at load (IDPF `http://www.idpf.org/2008/embedding`: SHA-1 of the OPF unique-identifier, whitespace-stripped, XOR the first 1040 bytes; Adobe `http://ns.adobe.com/pdf/enc#RC`: the identifier UUID hex as 16 key bytes, XOR the first 1024 bytes). `epub_encryption_is_drm` already classifies obfuscation vs. real DRM; this step makes the obfuscated fonts render as the publisher intended. Reference: `.foliate-js/epub.js` deobfuscators and Calibre's viewer.
