...
- [ ] **Screenshots** — Uncomment the `<screenshots>` block and add real images at stable raw GitHub URLs. The first entry must carry `type="default"`; that is what software centers feature. **`data/screenshots/` now exists** (commit `84108d1`, 2026-08-08) and holds four real captures — `reading.png`, `contents.png`, `search.png`, `reflow.png` — shot against the Calibre library on Hyprland. What is left is AppStream-specific: uncomment, point each `<image>` at a stable raw URL on `main`, add captions, re-run `appstreamcli validate --no-net`. The shots are 1500x980; Flathub prefers 16:9, so check whether they need recropping before submission.
- [ ] **Flatpak Manifest** — Build and verify `io.github.virinvictus.framework.yml` against the `org.gnome.Platform` 50 runtime, and test strict container compilation for MuPDF/DjVuLibre. **"GNOME 50" here is the Flatpak *runtime*, not a GNOME dependency** — GTK4 ships in `org.gnome.Platform` and is not in `org.freedesktop.Platform`, so every GTK4 app targets it regardless of libadwaita (see the GNOME-session-independence section below: Framework links no libadwaita since v0.80.0 and reads no `org.gnome.desktop.*` schema). The manifest already exists and already pins `runtime-version: '50'`; what has never been run is the build itself — no `build-flatpak/`, no `.flatpak-builder/`, and no installed `io.github.virinvictus.framework` (audited 2026-08-09). MuPDF builds from source here, so run it outside the terminal's memory-capped cgroup.
