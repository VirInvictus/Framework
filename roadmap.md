...
- [ ] LOW: Robustness: DjVu coords[4] uninitialized on short rect annotations; fz_register_document_handlers outside fz_try (pdf:331, cbr:588); miniexp walker recursion unbounded; MOBI guint16 truncation; CBR width/height cross-thread reads formally UB.
- [ ] LOW: Comment truth batch (19 items, FINAL-REPORT L5): window reflow-search fields that live in fw-webview, kinetic-scrolling ownership parenthetical, MOBI block-model ghosts, "FB2/TXT to come", residual FwReflowView refs, epub_encryption_is_drm cross-ref, cache header strategy/window numbers, "display lists", unreachable thumb_worker branch, CBR header threading paragraph, DjVu stale search comment pair, fw-state.h reflow_block dead field, fw-sandbox Phase-15 wording, zoom-storm header, fw-webview registry mechanism.
- [ ] LOW: Housekeeping: delete .claude/ + add /.claude/ and /.cache/ to repo .gitignore; relative compile_commands.json symlink + document the recreate step; prune framework-spec.md and claude.run .gitignore tombstones; i18n scaffold implies translatability that does not exist (localization = full string-marking pass).
