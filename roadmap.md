...
- [ ] **Live fit-width/fit-page recompute on resize.** `fw_view_fit_width_zoom` is only ever called from two sites: `apply_fit_width_tick`, the one-shot deferred call on document open (`fw-window.c:2076-2093`), and `on_doc_geometry_changed`, the CBR one-shot dimension-probe callback (`fw-window.c:2462-2479`). Neither is tied to a live size-allocate on `FwView` or its `GtkScrolledWindow`, so once a document is open, changing the window size never recomputes the active fit mode. On a floating GNOME window this rarely bites, since users resize rarely; on a tiling WM the window is resized constantly (a sibling opens, closes, or gets promoted), and Framework just keeps the stale zoom, over- or under-filling the new tile. Add a debounced resize hook in `fw-window.c` (the same `gtk_widget_add_tick_callback` idiom already used for the deferred initial fit) that reapplies `fw_view_fit_width_zoom`/`fw_view_fit_page_zoom` whenever `self->fit_width_active` (or an equivalent fit-page flag) is set and the scroll window's allocated width has changed since the last fit. Skip the recompute while the cache is in the `SCRUBBING` state so a resize storm doesn't also storm the render pool.
- [ ] **`GDK_TOPLEVEL_STATE_TILED` awareness in the spread auto-grow path.** `maybe_resize_for_spread` (`fw-window.c:291-342`) calls `gtk_window_set_default_size` to grow or shrink the window around a facing-pages spread, guarded only by `gtk_window_is_maximized` and `gtk_window_is_fullscreen` (`fw-window.c:302-304`). `GDK_TOPLEVEL_STATE_TILED` (plus the per-edge `_TOP_TILED`/`_RIGHT_TILED`/etc. flags) is present in the installed GTK4 headers and unused anywhere in `src/`. A tiled Hyprland window is neither maximized nor fullscreen, so this function still issues a size request the compositor owns on every spread transition. Add a `gdk_toplevel_get_state` check (via `gtk_native_get_surface`) alongside the existing guard so spread auto-grow is a no-op while tiled, the same way it already is while maximized.
- [ ] **`AdwOverlaySplitView` breakpoint for the TOC sidebar.** The sidebar (`fw-window.c:1902-1908`) is built with `adw_overlay_split_view_set_max_sidebar_width(self->split_view, 280)` and `show-sidebar` toggled by F9, but `collapsed` is never set and no `AdwBreakpoint` exists anywhere in `src/` (confirmed by grep). Without `collapsed`, opening the sidebar always squeezes the document view side by side: fine on a wide floating window, expensive on a half or third tile. Wire an `AdwBreakpoint` (or a manual width check against the window's allocation) that flips `collapsed` to `TRUE` below a narrow-width threshold (roughly 700 px, matching the usual libadwaita convention) so the sidebar overlays instead of squeezing once a tile narrows, and reverts automatically when the tile widens.
- [ ] **Quarter-tile dialog audit.** `AdwDialog` content sizes are hardcoded: Document Properties 480x540, Keyboard Shortcuts 520x640, Reading Settings 480x560 (`fw-window.c:1084-1087`, `1259-1262`, `1360-1363`). `AdwDialog` auto-degrades to a bottom sheet below its own internal breakpoint, so this is likely already fine, but it has never been exercised at genuine quarter-tile widths (roughly 340 px on a 1366-wide output split four ways). Manually verify all three dialogs at quarter- and third-tile widths on Hyprland; file a follow-up only if the bottom-sheet fallback actually clips content.
- [ ] **Continuous-scroll short-height check.** Walk the fixed-layout continuous-scroll view and the WebKit reflow view at short-height tiles (a horizontal split leaving roughly 300 px of height) to confirm page-boundary behavior, the reading ruler (F8), and the loupe (F7) overlays don't assume a tall viewport. No known bug here; this is a verification pass, not a rewrite, since `fw_view_snapshot`'s visible-range math (`fw-view.c:574-587`) is already viewport-height-relative and should degrade gracefully.

### Keyboard-first operation

- [ ] **Shortcut-table completeness audit.** Cross-checked the accelerator table in `fw-application.c:161-196` against the Keyboard Shortcuts dialog rows in `fw-window.c:1275-1334`; the two are already close to a 1:1 match (comic layout toggles, search navigation, reading overlays, rotation, printing all bound and documented). No gaps found in this audit. Re-run after any future feature lands: keyboard-only operation matters more without a GNOME Shell overview/app-switcher to fall back on.
- [ ] **Vim-style `j`/`k`/`gg`/`G` bindings: evaluated, not adopted.** Considered as part of this phase, since tiling-WM users skew toward keyboard-heavy workflows, but `CLAUDE.md`'s scope discipline explicitly excludes vim bindings, and Phase 14's own "Explicitly NOT borrowing" list already declines Zathura's vim keybindings by name. Recording the decision here rather than silently skipping it: Arrow keys, Page Up/Down, Home/End already cover the same navigation surface, and a second binding layer would cut against a stated non-goal. Not doing this.
- [ ] **Distraction-free windowed mode (no-chrome toggle).** `act_toggle_fullscreen` (`fw-window.c:732-740`) only calls `gtk_window_fullscreen`/`unfullscreen`; there is no windowed chrome-hide. True fullscreen pulls a tiled window out of the compositor's layout entirely (Hyprland treats it as its own fullscreen state layered on top of the tile), which is a bigger step than most tiling users want just to hide the header bar while reading in place. Add a lightweight toggle (new `win.toggle-chrome` action and accel) that hides `AdwHeaderBar`, and the TOC sidebar if open, without leaving the windowed/tiled state, restorable with the same key. Additive only: the existing F11 fullscreen behavior is untouched.

### GNOME-session independence

- [ ] **Confirm no `org.gnome.desktop.*` schema reads.** Audited: `src/*.c` has zero `g_settings_new("org.gnome...")` calls. Dark/light theming already goes through `AdwStyleManager`'s `notify::dark` signal (`fw-window.c:2034-2035`, `2291-2356`), which is portal-backed and desktop-agnostic. No action needed; recorded so a future contributor doesn't assume this is unaudited.
- [ ] **Reflow CSS font fallback chain: already safe, verify under Hyprland's fontconfig.** The shared reading CSS (`fw-reflow-html.c:15`) chains `'Crimson Pro', Georgia, 'Times New Roman', serif`, and `fw-fonts.c` registers bundled font directories with FontConfig independent of any GNOME first-run font provisioning. No code change; just confirm the bundle-registration path still resolves fonts correctly on a from-scratch Hyprland/Fedora session that never ran a GNOME session once.

### Wayland/Hyprland integration

- [ ] **App ID / `.desktop` basename match: already correct, worth documenting.** `APP_ID` (`meson.build:16`, `io.github.virinvictus.framework`) is passed as `application-id` in `fw-application.c:221` and matches `data/io.github.virinvictus.framework.desktop.in`'s basename, so a Hyprland `windowrulev2 = ..., class:^(io.github.virinvictus.framework)$` rule already works. No code change needed; this item exists to call out the Hyprland-relevant fact explicitly rather than leave it assumed.
- [ ] **Fractional-scaling re-verification under Hyprland.** The render-scale-matches-surface-scale fix already shipped (Phase 2, "Wayland Fractional Scaling," `fw_cache_set_scale_factor`), but it was validated under Mutter. Hyprland's `wp-fractional-scale-v1` implementation has had its own history of quirks across versions. Re-run the blurry-at-1.25x/1.5x check specifically on Hyprland before calling this closed for the new desktop.
- [ ] **Portal usage: already conformant.** File open (`gtk_file_dialog_new`, `fw-application.c:73`, `fw-window.c:886`) and external-link launch (`gtk_uri_launcher_new`, `fw-view.c:1313`) are both portal-mediated GTK4 APIs already; no legacy `GtkFileChooserDialog` remains in the tree. No action; recorded as verified rather than assumed.
- [ ] **`xdg-activation` smoke test.** GTK4/Wayland handles activation-token focus-stealing prevention automatically when a file manager or terminal launches Framework, but this has only been exercised under GNOME Shell/Mutter. Verify a file manager "Open With" action (or `gio launch`) correctly raises and focuses the Framework window on Hyprland rather than leaving it unfocused behind the launcher.

### CSD posture

- [ ] **`gtk-decoration-layout` / hidden-buttons sanity check.** Framework's `AdwHeaderBar` relies on the toolkit-level `gtk-decoration-layout` `GtkSettings` property; no Framework-side override exists (confirmed by grep). Under GNOME this value is populated from `org.gnome.desktop.wm.preferences`, which a bare Hyprland session may not have installed at all (no `gnome-settings-daemon`). Verify the header bar still renders sane window-control buttons, or gracefully shows none, when that schema is entirely absent, and that nothing crashes on the missing-schema path.
...
- [x] **Either path: Phase 18's audit items are the verification tail.** Portal dark/light, hidden-button CSD, and the tiling-friendly floating sidebar are exercised by the v0.80.0 migration. The remaining Phase 18 code items (live fit-recompute on resize, `GDK_TOPLEVEL_STATE_TILED` spread guard, no-chrome toggle) are now easier to land on the owned shell and stay tracked under Phase 18.
