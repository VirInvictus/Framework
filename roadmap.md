...
- [x] **AppStream Metadata** — Written already (`data/io.github.virinvictus.framework.metainfo.xml.in`, 122 lines) and it passes `appstreamcli validate --no-net` clean, with three screenshots, description, and categories. **One gap remains, tracked separately: the `<releases>` history is stale.** It runs 0.6.0 through 0.12.0 and then lists a `1.0` entry that has not shipped, while `meson.build` reads 0.82.0. Left as-is rather than dumped full, because AppStream wants meaningful milestones rather than every 0.x point release; the curation decision (and deleting the phantom 1.0) belongs to the 1.0 push.
