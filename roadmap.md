...
*MuPDF opens everything in the anti-library.*
...
- [x] **Symmetric ±10-page parsed window with toggleable kinetic scrolling**: Priority window is now visible + 10 forward + 10 backward, interleaved, in all non-SCRUBBING states. A capture-phase `GtkEventControllerScroll` on `FwView` caps each scroll event at 90 px (wheel ticks scaled via `SCROLL_WHEEL_STEP = 60` first), applied directly to the vadjustment; this replaces GTK's default kinetic momentum scrolling. The cap is gated by a new GSettings key `kinetic-scrolling` (bool, default false) wired to a primary-menu toggle "Kinetic Scrolling" via `g_settings_create_action`; flipping the menu item switches behavior live. Default off = cache-friendly (the v0.14 default; FLIPPED to default-on in v0.25.0 - the schema is authoritative). (v0.14)
...
- [x] **Corpus manifest**: `tests/corpus.json` holds the canonical sample list (default root `/home/bdkl/docs/Calibre Library/`, env override `FW_TEST_CORPUS_ROOT`). Tagged samples (`large`, `textbook`, `djvu`, `scanned`, `reflow`). Currently consumed by `stress-scrub` via argv; the manifest reader for fully manifest-driven tests lands as bench/soak tests come in. (v0.15)
  - **Correction (2026-09-13):** this box's default-root framing is historical. The shipped default corpus root is the repo's gitignored `.testfiles/` (set in `tests/stress/meson.build`; `FW_TEST_CORPUS_ROOT` overrides at runtime per invocation). The Calibre Library is only the ad-hoc debugging source.
...
- [~] **Foliate-Style Reflow Rewrite**: Native-GTK reflow for EPUB / MOBI / AZW3 / FB2 / TXT, bypassing MuPDF's fixed-layout engine for these formats. Design locked in `docs/foliate-rewrite.md`, which covers format scope (LIT/CHM/DOCX/RTF explicitly skipped), `FwReflowDocument` interface, `FwReflowView` GtkListView pattern, format-specific implementation notes for each backend, and a 6-phase shipping plan. Format-parsing canon: `.foliate-js/`'s `mobi.js` / `epub.js` / `fb2.js`. (Earlier patchnotes call this "Fractal-style", a slip renamed to "Foliate-style" in v0.51.0.)
  ...
  - [x] **Phase 2: `FwReflowDocumentFb2`** (v0.41.0; libxml2 rewrite v0.57.0). v0.41.0: GMarkupParser-based walker producing CHAPTER markers + nested HEADING + PARAGRAPH (with inline Pango markup) + BLOCKQUOTE + HR + IMAGE (resolved against `<binary>` base64 → `GdkTexture`). v0.57.0 swap: rewritten as a libxml2 tree walker mirroring foliate's STYLE / SECTION / POEM / BODY transform tables in C dispatch. Three-pass open (metadata → binaries → body); same inline-stack auto-close pattern as EPUB / MOBI; encoding declarations handled natively by libxml2. Bare `.fb2` only; `.fb2.zip` is a follow-up.
  - [x] **Phase 3: `FwReflowDocumentEpub`** (v0.42.0; libxml2 + nav.xhtml in v0.56.0). Original v0.42.0: libarchive ZIP read → container.xml → OPF → manifest + spine + `dc:` metadata; per-chapter XHTML walked through GMarkupParser into HEADING / PARAGRAPH / BLOCKQUOTE / CODE / IMAGE / HR / LI blocks; NCX → TOC; manifest images → `GdkTexture` hash keyed by resolved zip path and manifest id. v0.56.0 swap: chapter walker switched to libxml2's `htmlReadMemory` (tolerant of malformed XHTML, matches foliate's DOMParser); EPUB 3 `nav.xhtml` added as TOC fallback when NCX is absent or empty (foliate's `parseNav` ported to C). Tested clean on five Calibre EPUBs.
  - [x] **Phase 4: `FwReflowDocumentMobi`** (v0.52.0). PalmDB envelope + PalmDOC LZ77 + EXTH metadata, transcribed line-by-line from `.foliate-js/mobi.js` with explicit C guards mirroring foliate's accidental JS-runtime tolerance (LZ77 bad back-refs emit 0; trailing-data over-counts clamp to empty). Tag-soup balancer pre-pass handles MOBI's malformed HTML (orphan closes, unclosed voids, unquoted attribute values). Heuristic pagination kicks in at ≥ 800 blocks so 5 000-block MOBIs render immediately. Verified on three real Calibre MOBIs (Ryder-Hanrahan, Gemmell ×2). KF8 / AZW3 / HuffDic / MOBI image extraction / NCX TOC are deferred to follow-up slices.
  - [x] **Phase 5: `FwReflowDocumentAzw3` / KF8** (v0.55.0). KF8 boundary detection (combo MOBI/KF8 + pure AZW3); INDX parser ported from foliate's `getIndexData` (TAGX schema, bit-controlled variable-length tag values); SKEL + FRAG splice ported from `loadSection` to produce real HTML from the spliced sections. HTML walker switched to libxml2's `htmlReadMemory` (tolerant of malformed markup, matches foliate's DOMParser). Inline-tag-stack auto-close at block boundaries fixes `<a>`-wrapping-`<p>` markup. Verified on three real Calibre AZW3s (Wood, Krebs, Feeney). KF8 TOC now extracted via the NCX index (v0.67.0, see below); HuffDic compression remains untested-against-a-real-file (none in the corpus) but is decoder-hardened.
  - [x] **Phase 6: Polish** (completed v0.67.0). Search-hit highlight in the GtkListView (shipped in the v0.67.0 release commit, recorded as the v0.66.0 patchnotes entry; synchronous block-model search, markup-aware splice, F3/Shift+F3 cycling); font-size adjustment (Ctrl+ / Ctrl-); chapter sidebar, including **KF8/AZW3 NCX TOC** (v0.67.0; foliate `getNCX` port: NCX INDX + CNCX labels + `[fid,off]`→spliced-body-offset mapping + anchor injection) and **complete nested-navPoint EPUB TOCs** (v0.67.0); **"% read" progress indicator** (v0.67.0); **"Render as Fixed Pages"** fall-through-to-MuPDF toggle (v0.67.0; `prefer-fixed-layout` GSetting, re-opens the document).
...
## Phase 14: UX Polish (The "Sumatra Clone" Pass)
*Features identified across reference repositories, translated into native GTK/GNOME idioms.*
...
- [x] **Fit-width page-size normalization** (v0.69.0). Fit-width keys off the median body-page width (cover and spreads excluded, via aspect ratio or the CBR filename signal) instead of the current or widest page, so an anomalous cover or a centerfold no longer shrinks the rest of the document. Computed for free in `recompute_layout`'s existing per-page loop. CBR (whose pages default to the cover's size at open, RAR being a sequential stream) learns real per-page dimensions via a one-shot background probe that emits the new `FwDocument::geometry-changed`, prompting a relayout and re-fit. Backend-agnostic (PDF / DjVu / comics / XPS).
...
- [x] **MOBI / AZW3 `produce_html`** (Phase 17.2, v0.72.0): the KF7/KF8 reconstructed HTML (with TOC markers) is stitched and handed to the WebView; MOBI/AZW3 inherit the v0.71 typography + themes. Shared `fw-reflow-html` module (reading CSS, find-body, script-strip, img-rewrite via a resolver callback); parser retains raw image bytes; img refs resolved for KF7 zero-padded `recindex` and KF8 base-32 `kindle:embed:`; EXTH-only cover emitted as a synthetic section.
- [x] **FB2 `produce_html`** (Phase 17.3, v0.73.0): a recursive FB2-XML-to-HTML emitter (sections with id anchors, titles/subtitles to headings by depth, paragraphs, inline emphasis/strong/code/sub/sup, poems/verses and citations to blockquotes, empty-line to `<hr>`, `<image>` to `framework-img:`). Parser retains the xmlDoc + raw `<binary>` bytes; reuses the shared `fw-reflow-html` reading CSS.
- [x] **TXT `produce_html`** (Phase 17.4, v0.74.0): reuses the blank-line paragraph split, emits a `<p>` per paragraph (hard newlines to `<br>`), wrapped with the shared reading CSS. Last reflow format on WebKit.
- [x] **Markdown (`.md`) via WebKit** (v0.75.0): net-new format. `FwReflowDocumentMd` runs the file through vendored md4c (MIT, GitHub dialect: tables, task lists, strikethrough, autolinks) to HTML, with `MD_FLAG_NOHTML` to block raw-HTML/script injection; shared reading CSS extended (theme-adaptive tables / code / task lists). Local images + code syntax highlighting are follow-ups.
- [x] **Delete `FwReflowView`** (Phase 17.5a, v0.76.0): the block-model GtkListView renderer is removed now that every format is on WebKit. `fw-reflow-view.{c,h}` deleted; `fw-window.c` unified on the single `"webview"` reflow path (dual stack pages, parallel nav/search/save branches, and the `reflow_hits` / `reflow_active` / `reflow_search_*` / `on_reflow_page_changed` machinery all gone). `FwReflowSidebar` retained.
- [x] **Strip the block-model machinery** (Phase 17.5b): drop the now-unused `get_block_model` / `get_image` / `find_block_by_anchor` vtable slots, `fw_reflow_document_search`, the `FwBlock` / `FwReflowHit` types, and each backend's block-building walk. Entangles with the live `produce_html` path in EPUB / MOBI (one parse loop feeds both), so it needs a careful per-backend separation and visual verification. `stress-reflow.c` (the only block-model API consumer) gets rewritten to assert on `produce_html` output instead.
- [x] **Security tightening** (Phase 17.x, v0.78.0): all three items closed. (1) The Landlock EXECUTE drop is restored behind path-beneath allows for the WebKit helper dir (pkg-config-derived at build, distro-layout fallbacks at runtime) plus the loader dirs; all-or-nothing ruleset build falls back to MAKE_*-only if the helper dir is missing, and `FW_NO_LANDLOCK=1` is the debugging escape hatch. (2) HTML emit now scrubs active content: `<iframe>`/`<object>`/`<embed>` stripped with `<script>`, all `on*` attributes removed, `javascript:`-scheme URL attributes removed (whitespace-tolerant scheme parse); `stress-reflow` asserts it with a synthetic hostile EPUB. (3) The bubblewrap sandbox question is settled, not deferred: a probe proved our own Landlock ruleset (which denies mount-topology changes to all children) is what breaks bwrap, so the two are mutually exclusive; decision is Landlock stays (it covers the in-process MuPDF/DjVuLibre/libarchive parsers), WebKit sandbox stays disabled, rationale documented in `main.c`.
- [x] **Publisher CSS + cross-chapter links** (Phase 17.x, v0.79.0): shipped, closing Phase 17. Publisher stylesheets are served by zip path through `framework-img://<doc-id>/res/<path>` (one scheme rather than the planned `framework-css:`; same-origin is required for `@font-face`, and hierarchical paths let relative `url()`/`@import` resolve natively with zero CSS rewriting), collected/deduped from chapter heads, emitted ahead of the reading CSS whose body rules now carry `!important` so theme/typography stay user-sovereign; gated by a live `publisher-styles` toggle (default on). EPUB font de-obfuscation shipped exactly as specced (both algorithms; keys from the now-tracked `package@unique-identifier` linkage and UUID identifier; verified on Gone Girl + a synthetic IDPF round-trip). Cross-chapter links: chapter ids namespaced `sN-*`, hrefs rewritten to in-doc fragments, TOC anchors translated to DOM ids (this fixed outright-broken EPUB TOC navigation), MOBI `filepos` and KF8 `kindle:pos` links repaired (fragment table exported from the parser), FB2 note links emitted as real anchors. A `decide-policy` hook keeps the stitched book from being navigated away (external links open in the browser) and a WebKit content filter blocks all remote subresource fetches. Deferred follow-ups (both shipped in v0.82.0, closing them out):
  - [x] **KF8 flow CSS** (v0.82.0). The MOBI parser now reads the FDST record (KF8 header offset 192) and slices the auxiliary flows out of the decompressed stream before the SKEL+FRAG splice (they used to be discarded). The MOBI backend serves each as `flow/<i>.css` through the existing `framework-img://<doc>/res/` scheme, hoists the per-section `<head>` `kindle:flow:<id>` stylesheet links (base-32 id → flow index) into the stitched shell as `class="fw-pub"` links ahead of the now-id-tagged reading CSS, and implements `get_resources`. Verified on real AZW3s (Spam Nation, ACOMAF: two real stylesheets each) and ASan-clean; KF7 `.mobi` unaffected (no flows). The reading CSS keeps its `!important` sovereignty and the `publisher-styles` toggle now works on KF8 too.
  - [x] **Dark-theme color transformation** (v0.82.0). `fw_webview_set_dark_transform` injects a Calibre-style pass (gated on a dark reading theme) that inverts the *lightness* in HSL (keeping hue and saturation) of author-set light backgrounds (→ dark) and dark text (→ light), leaving correct-for-dark colors untouched. Two-pass (read all computed colors, then apply) to avoid layout thrash; idempotent and reversible via a `data-fw-dark` tag; re-runs on theme change and the publisher toggle. Applies to every reflow format (EPUB + KF8).
...
- [x] **Application Assets** (2026-07-29). The `.desktop` file was already complete and passes `desktop-file-validate` clean; the gap was the artwork. The scalable app icon was still painted in Adwaita blue (`#3584e4`/`#1a5fb4`) three releases after v0.80.0 dropped libadwaita, so it was repainted in the app's own Kanagawa Dragon set (`dragonBlack0` base, `dragonBlue2` frame and heading rule, `dragonWhite` page) with a hairline accent edge so it still reads against a dark dock. `data/icons/hicolor/symbolic/apps/` was an empty directory: it now holds `io.github.virinvictus.framework-symbolic.svg` (the `-symbolic` suffix is what makes GTK recolour it), a single-colour folded page on the 16px grid inside the conventional 2px margin, whole-pixel rules, and no outer frame because the frame collapses into the page edge at that size. `data/icons/meson.build` installs the symbolic icon alongside the scalable one. `logo.svg` carried a copy of the same artwork and had drifted to the old blue; it now pulls the icon in through a nested `viewBox` so the two cannot diverge again.
- [x] **AppStream Metadata**: `data/io.github.virinvictus.framework.metainfo.xml.in` passes `appstreamcli validate --no-net` clean, with description, categories, launchable, content rating, and the recommends/supports blocks. The `<releases>` history was reconciled on 2026-08-08: it had run 0.6.0 through 0.12.0 while `meson.build` read 0.82.0, roughly seventy releases behind. The **curation policy is now recorded in a comment above the block**: milestones only, never every point release, and never a version that has not shipped. Seventeen entries now span 0.6.0 to 0.82.0, each one a change a reader would actually notice.
  - Two claims this line previously carried were wrong and are corrected here. There was **no phantom `1.0` entry** to delete (checked against the file's full git history; one was never written), and the file **does not carry three screenshots** (the `<screenshots>` block is still the original commented-out placeholder). Screenshots remain genuinely outstanding for the 1.0 push, and `data/screenshots/` does not exist yet.
  - *(Superseded 2026-09-04 by the Screenshots box below: the block is wired with four real captures and `data/screenshots/` exists.)*
...
- [x] **Live fit-width/fit-page recompute on resize (v0.81.0).** Both fit modes now track the viewport. A `fit_page_active` flag mirrors `fit_width_active` (both cleared in `set_zoom`, re-armed by their callers); the scroll's h/v adjustments' `"changed"` signal (page-size tracks the viewport extent) drives a 130 ms-debounced `refit_after_resize` that reapplies the active fit when the viewport dimensions actually changed. The dim-equality guard makes a zoom-triggered `"changed"` a no-op, so there's no feedback loop and no resize-storm on the render pool (the debounce coalesces the storm rather than querying `SCRUBBING`).
- [x] **`GDK_TOPLEVEL_STATE_TILED` awareness in the spread auto-grow path (v0.81.0).** `maybe_resize_for_spread` now checks `gdk_toplevel_get_state` (via `gtk_native_get_surface`) for `GDK_TOPLEVEL_STATE_TILED` alongside the existing maximized/fullscreen guard, so spread auto-grow is a no-op while tiled and no longer issues a size request the compositor owns.
- [x] **TOC sidebar never squeezes the page (obsolete after de-adwaita; verified Hyprland 0.55.4, 2026-07-17).** This item's premise (an `AdwOverlaySplitView` that squeezes the document side by side, wanting an `AdwBreakpoint` to collapse it on narrow tiles) went away with v0.80.0: the sidebar is now a *floating* `GtkRevealer` in a `GtkOverlay` (F9, click-outside dismiss) that overlays the page and never reallocates it, which is exactly the narrow-tile behavior the breakpoint would have provided. No breakpoint needed.
- [x] **Quarter-tile dialog audit (premise obsolete; verified Hyprland 0.55.4, 2026-07-17).** The `AdwDialog`-embedded-in-the-window / bottom-sheet-fallback concern went away with v0.80.0: the three dialogs (Reading Settings, Document Properties, Keyboard Shortcuts) are now modal transient `GtkWindow`s. Verified live that Hyprland floats all three as their own windows (491×560 / 480×560 / 520×640) with the main window still tiled, so a narrow tile never squeezes or clips them. No crash opening any of the three.
- [~] **Continuous-scroll short-height check (no-crash verified; pixel confirmation still Brandon's).** Launching and driving the fixed-layout and reflow views at short-height tiles is crash-clean, and the visible-range math (`fw_view_snapshot`, `fw-view.c:574-587`) is viewport-height-relative by construction. The remaining piece is a genuinely visual one I can't do headlessly (no pixel readback): eyeball the reading ruler (F8) and loupe (F7) overlays at a ~300 px-tall tile to confirm they don't assume a tall viewport. No code change expected.

### Keyboard-first operation

- [x] **Shortcut-table completeness audit (re-run 2026-07-17).** The accelerator table (`fw-application.c`) and the Keyboard Shortcuts dialog rows (`fw-window.c`) stay a 1:1 match: comic layout toggles, search navigation, reading overlays, rotation, and printing are all bound and documented, and the new F12 "Hide chrome" toggle (v0.81.0) was added to both in the same change. No gaps. Re-run after any future feature lands.
- [x] **Vim-style `j`/`k`/`gg`/`G` bindings: evaluated, not adopted (decision recorded).** Considered as part of this phase, since tiling-WM users skew toward keyboard-heavy workflows, but `CLAUDE.md`'s scope discipline explicitly excludes vim bindings, and Phase 14's own "Explicitly NOT borrowing" list already declines Zathura's vim keybindings by name. Recording the decision here rather than silently skipping it: Arrow keys, Page Up/Down, Home/End already cover the same navigation surface, and a second binding layer would cut against a stated non-goal. Not doing this.
- [x] **Distraction-free windowed mode (no-chrome toggle) (v0.81.0).** New `win.toggle-chrome` action (F12, mnemonic sibling of F11) hides the `GtkHeaderBar` titlebar and, if open, the floating TOC (remembering it was open so the same key restores both), without leaving the windowed/tiled state. Additive: F11 fullscreen is untouched. Listed in the Keyboard Shortcuts dialog under View.

### GNOME-session independence

- [x] **No `org.gnome.desktop.*` schema reads (re-verified 2026-07-17).** `grep` of `src/` confirms zero `g_settings_new("org.gnome...")` calls. Since v0.80.0 dark/light theming goes through `fw-theme.c`'s `org.freedesktop.portal.Settings` reader (`notify::dark`), not the retired `AdwStyleManager`; it is portal-backed and desktop-agnostic. The only GSettings schema Framework reads is its own (`io.github.virinvictus.framework`).
- [x] **Reflow CSS font fallback chain: safe under Hyprland (verified 2026-07-17).** The shared reading CSS chains `'Crimson Pro', Georgia, 'Times New Roman', serif`, and `fw-fonts.c` registers the bundled font dirs with FontConfig independent of any GNOME first-run provisioning. Reflow rendering exercised on real EPUB/AZW3/MOBI in this session's Hyprland runs (text laid out correctly, no missing-font fallback boxes). The bundle-registration path is desktop-agnostic; nothing GNOME-specific in it.

### Wayland/Hyprland integration

- [x] **App ID / `.desktop` basename match (verified live, Hyprland 0.55.4, 2026-07-17).** `hyprctl clients` reports the window `class` as exactly `io.github.virinvictus.framework`, matching `APP_ID` and the `.desktop` basename, so a Hyprland `windowrulev2 = ..., class:^(io.github.virinvictus.framework)$` rule targets it correctly.
- [ ] **Fractional-scaling re-verification under Hyprland (Brandon's bench; needs a scale change).** The render-scale-matches-surface-scale fix shipped in Phase 2 (`fw_cache_set_scale_factor`), validated under Mutter; the plumbing is scale-factor-driven and desktop-agnostic (unchanged since). Can't verify headlessly: the dev machine's `eDP-1` is at `scale: 1.00`, and confirming crisp rendering at 1.25×/1.5× means changing the display scale (a whole-session rescale) and eyeballing sharpness, which is a Brandon task. This is the one open Phase 18 item that genuinely needs a human at the display.
- [x] **Portal usage: conformant (verified 2026-07-17).** File open (`gtk_file_dialog_new`) and external-link launch (`gtk_uri_launcher_new`) are both portal-mediated GTK4 APIs; `grep` confirms no legacy `GtkFileChooserDialog` in the tree.
- [x] **`xdg-activation` smoke test (verified Hyprland 0.55.4, 2026-07-17).** Launched from a terminal; `hyprctl activewindow` reports the Framework window as active (`focusHistoryID: 0`) immediately on map, so it raises and focuses correctly rather than opening unfocused behind the launcher. GTK4/Wayland's activation-token handling works as expected under Hyprland.

### CSD posture

- [x] **`gtk-decoration-layout` / hidden-buttons sanity check (verified 2026-07-17; premise softened by de-adwaita).** The point is now moot by construction: since v0.80.0 the flat `GtkHeaderBar` sets `gtk_header_bar_set_show_title_buttons(FALSE)` unconditionally (the compositor owns window controls; Ctrl+Q quits), so Framework renders *no* title buttons regardless of what `gtk-decoration-layout` resolves to. It therefore can't be broken by a missing `org.gnome.desktop.wm.preferences`. Launched clean on Hyprland with no crash. (That schema does happen to be installed on this box via the GNOME-fallback session, so the truly-absent path isn't exercised here, but Framework reads neither it nor the decoration layout in any load-bearing way.)
- [x] **No SSD reliance (verified live, Hyprland 0.55.4, 2026-07-17).** Post-de-adwaita the window is a stock `GtkApplicationWindow` with a plain `GtkHeaderBar` (client-side decorations); no server-side-decoration assumption anywhere in `src/`. `hyprctl` confirms the window renders CSD with no crash and no missing controls.

---

## Phase 19: Hyprland-Native Shell (De-adwaita, or the Zig Rewrite; post-1.0, decision gate first)

*Portfolio direction change (Brandon, 2026-07-09): the goal is an app that fully belongs on Hyprland, which for the rest of the portfolio means dropping libadwaita while keeping GTK4, with Colophon as the pilot (its roadmap, Phase 6). Framework has an extra fork Brandon put on the table the same day: he was considering rewriting Framework in Zig.*

*Resolution (2026-07-16, shipped v0.80.0): the Zig rewrite is **deferred, not chosen**: Framework stays C17/GTK4 and Path A shipped. The original "nothing starts before Phase 15" sequencing was overridden: de-adwaita landed **before** the 1.0 release so 1.0 goes out already Hyprland-native rather than as a libadwaita app.*

- [x] **Decision gate: Zig rewrite go/no-go; verdict: no rewrite (deferred).** Keeping the working C17/GTK4 app (with its shipped Flatpak and WebKitGTK reflow path) beat a rewrite whose cost landed against a moving, feature-complete target. The Zig option stays on the table as a much-later possibility, not a near-term plan; it would need its own research pass and spec if ever revived.
- [x] **Path A (no rewrite): de-adwaita the C17 codebase (v0.80.0).** libadwaita dropped, plain GTK4 kept. New `fw-theme.c` owns a portal `org.freedesktop.portal.Settings` `color-scheme` reader (dark default, live `SettingChanged`) plus the two CSS providers; new `data/style.css` is the owned Kanagawa Dragon/Lotus stylesheet at `PRIORITY_USER + 1`. Every `Adw*` replaced: `AdwApplication`/`AdwApplicationWindow` → `Gtk*`; `AdwHeaderBar` → a flat `GtkHeaderBar` titlebar with hidden window buttons (Ctrl+Q quits); `AdwOverlaySplitView` → a **floating** `GtkOverlay`+`GtkRevealer` TOC (F9, click-outside dismiss) that never squeezes the page; `AdwToast` → an owned bottom-center revealer; the three `AdwDialog`s → modal `GtkWindow`s built from owned `GtkListBox` boxed-list rows (new `fw_dialog_new`/`fw_pref_group`/`fw_value_row` helpers); `AdwAlertDialog` → `GtkAlertDialog`; `AdwAboutDialog` → `GtkAboutDialog`; `AdwStatusPage` → an owned centered box. `grep -ri adw src/` is clean; the binary no longer links libadwaita. No behavior regression under GNOME; the look changed, not the function.
- [ ] **Path B (rewrite): fold this phase into the rewrite's spec.** Not taken (Path A shipped). Retained only as the shape a future Zig rewrite would follow.
- [x] **Either path: Phase 18's audit items are the verification tail.** Portal dark/light, hidden-button CSD, and the tiling-friendly floating sidebar are exercised by the v0.80.0 migration. The three Phase 18 code items (live fit-recompute on resize, `GDK_TOPLEVEL_STATE_TILED` spread guard, no-chrome toggle) all landed in v0.81.0, and the audit tail was closed out on real Hyprland 0.55.4 (2026-07-17): fit-recompute confirmed by watching the zoom track live width changes, the spread guard by a GTK probe showing `gdk_toplevel_get_state` reports `TILED` for a tiled window (Hyprland also flags it `MAXIMIZED`, so the pre-existing guard already covered it), floating dialogs, correct window class, and launch focus all verified. Only two Phase 18 items stay open, both needing a human at the display: the short-height overlay eyeball and fractional-scaling sharpness at 1.25×/1.5×.

## Audit Findings (code review, post-reflow-search)
...
- [ ] **Split `fw-window.c` (2,934 lines at the final-audit pass; 2,936 after the v0.83.0 dialog-helpers extraction; 2,551 when this box was written, 3,030 at the Phase 20 sweep)**: cleanly sectioned but large. The dialog/row helpers moved out in v0.83.0 (`fw-dialog-widgets.c`); remaining extraction candidates are the ~400-line `build_ui` constructor (→ `build_header_bar` / `build_search_bar` / … helpers) and the action handlers. Cohesive as-is, so this is polish. (Explicitly deferred in v0.67.0: high churn / low value, best done alongside the Phase 15 1.0 restructuring.)
...
- [x] **FB2 body `<title>` metadata fallback**: fills the title when `<title-info>` is absent.
- [x] **EPUB NCX parent entries**: nested navPoints now flush at `<content>`, so parent (section) entries reach the TOC in reading order (the leaf-only gap is closed; the associated leak was fixed too).
- [x] **`.fb2.zip`** support (libarchive extract of the inner `.fb2`).
- [x] **AZW3 / KF8 NCX TOC** (see Phase 13.1 Phase 6).
- [x] **Coalesced redraw scheduling**, **lockless cache-field reads removed**, **`RENDER_BYTES_CAP` hoisted to file scope** (`fw-cache.c`).

### Done in the audit pass (carried by v0.67.0)
- [x] **Perf (tier 1): removed `set_priority` per-tick allocation churn** (`fw-cache.c`). Replaced the keep-set `GHashTable` (rebuilt on every scroll tick) with a linear membership scan over the ≤64-page priority window, and dropped the per-call `to_remove` array by stealing parsed entries during iteration. Only the conditional byte-cap `victims` array remains, allocated solely when over cap. Verified clean under ASan (`stress-scrub` exercises both eviction tiers under a tight cap).
- [x] **Perf (tier 2): search overlay is now zero-copy** (`fw-view.c::fw_view_snapshot`). Scans `fw_search_peek_hits` and filters by page inline instead of allocating a per-page GArray every frame; removed the now-dead `fw_search_hits_for_page`.
- [x] Added missing `#include <math.h>` to `fw-cache.c` (`sqrt`/`fabs` resolved only via transitive GTK includes).
- [x] Removed dead `last_priority_time` field + write + misleading comment from `fw-cache.c`.
- [x] Corrected the inaccurate `fw_search_peek_hits` doc comment.
- [x] **Security: hardened the MOBI record-offset table** (`fw-mobi-parser.c`). Offsets are read straight from the file and were unvalidated; a non-monotonic table made `roff[i+1] - roff[i]` underflow to a huge `gsize`, defeating the length-relative bounds checks (notably the HuffDic decoder) and allowing an out-of-bounds read on a crafted MOBI. Now rejects any non-monotonic or out-of-range table. Verified valid KF7/KF8 files still open unchanged via `stress-reflow`.
- [x] **Leak: fixed NCX `cur_src` overwrite leak** (`fw-reflow-document-epub.c`). Nested navPoints overwrote the content src without freeing the prior value, leaking once per nested entry (~28 KB on a deep TOC). Caught by the new ASan-capable `stress-reflow`, the reflow pipeline's first automated leak coverage.
- [x] Test suite: reproducible `.testfiles/` corpus + shared `corpus-root.h` / `stress-util.h`, new `stress-reflow` target, deduped helpers, trimmed unused includes.

### Reviewed
- `fw-document-pdf.c` is clean: threading model, `fz_try/catch` discipline, `fz_cookie` cancellation (real cache path), stext LRU cache, dispose/finalize ordering all sound.
- `fw-reflow-document-epub.c`: resource lifecycle otherwise correct; the one NCX leak `stress-reflow` surfaced is fixed (above), and the leaf-only-TOC behavior is noted under Remaining.
...
- [x] **Commit the in-flight v0.77.0 work.** Committed as `02c8a44` (v0.77.0, 2026-06); verified clean working tree 2026-07-16.
- [x] **Clear the script litter at repo root.** `modify_*.py` helpers are gone from the tree (verified 2026-07-16).
- [x] Gitignore or remove the untracked `.antigravitycli/` directory. Gone from the tree (verified 2026-07-16).

---

## Deferred (v2.0+ / Strictly Out of Scope for 1.0)
*Listed so the architecture doesn't preclude them. 1.0 has shipped; these ranks now live in Phase 21 above (single-page is rank 8, thumbnail sidebar rank 7, presentation rank 16, configurable keybindings rank 15). This section records the original post-1.0 list.*
...
## Phase 21: Post-1.0 Feature Lane (final audit, ranked)
*The audit's lens-4 proposals, slotted in recorded rank order (2026-09-15). Boxes are the lane, not a schedule: work starts top-down when the blitz closes. §13 was amended the same day (see spec): continue-reading recents and navigation bookmarks are ruled viewer state, unblocking ranks 12-13.*

- [ ] **Reflow auto-reload parity (rank 1)** — the fixed-layout monitor/debounce/toast machinery shipped v0.21; the reflow branch skips it (`fw-window.c`). Completing a shipped feature: attach the monitor on the WebView path and re-open with position restore. The README auto-reload claim is already qualified until this lands.
- [ ] **PDF page labels (rank 2)** — `fz_page_label` exists in the linked MuPDF; the header shows bare "%d / %d" (`fw-window.c`). Real books carry front-matter numbering; surface labels in the page entry, Go-to-page, and the sidebar where present.
- [ ] **Window geometry persistence (rank 3)** — 900x700 hardcoded every launch (`fw-window.c`); store last geometry in state/GSettings and restore maximized/fullscreen state. The most frequent daily papercut a viewer can have.
- [ ] **Recolor light/dark keys (rank 4)** — `recolor-light` / `recolor-dark` GSettings keys plumbed into the existing color-matrix constants (the recorded v0.22 follow-up; the full zathura-style customization).
- [ ] **Reading measure control (rank 5)** — `--measure: 38em` hardcoded in the reading CSS (`fw-reflow-html.c`); one schema key + one Reading Settings row benefits every reflow format.
- [ ] **Reflow print parity or honest disable (rank 6)** — Ctrl+P is a silent no-op on reflow formats (`act_print` early-returns; reflow never sets `self->document`). Either `WebKitPrintOperation` (already linked) or a visible "not printable" message; decide at implementation time.
- [ ] **Thumbnail sidebar (rank 7)** — the expensive half (persistent thumbnail tier) shipped Phase 2; a GtkGridView rail alongside the TOC fixes comics and scanned PDFs. Rotation rendering stays at 0 per the polish note below.
- [ ] **Single-page view mode (rank 8)** — the state schema already persists an unconsumed `view_mode` key; prerequisite for presentation mode. Lives in Deferred below until this box opens.
- [ ] **stext render-time pre-warm (rank 9)** — the recorded Sumatra `RenderCache.cpp:790` follow-up (Phase 11 Tier 1 box): extract stext during render-worker success. Touches the render hot loop; stress suite after.
- [ ] **Markdown local images + syntax highlighting (rank 10)** — local images ride the existing `framework-img://` scheme; highlighting is decided (2026-09-15, Brandon): hand-rolled fenced-code tokenizer, no new dependency and no second vendored surface.
- [ ] **Pinch-to-zoom (rank 11)** — `GtkGestureZoom` into the existing fit machinery; gesture-arbitration risk noted at audit time.
- [ ] **Continue-reading recents (rank 12)** — UNBLOCKED by the 2026-09-15 §13 amendment: a short continue-reading list on the empty window sourced from state.json's timestamped entries. No library UI.
- [ ] **Navigation bookmarks (rank 13)** — UNBLOCKED by the same ruling: named page marks stored only in state.json. Anything writing into the document stays banned.
- [ ] **DjVu margin-crop support (rank 14)** — F6 silently does nothing on DjVu (`get_content_bbox` returns FALSE), precisely the scanned-document format crop exists for.
- [ ] **Configurable keybindings (rank 15)** — a centralized accel table makes it bounded; brushes the recorded Phase 18 vim-binding rejection (that rejection was about vim-style bindings, not user config).
- [ ] **Presentation mode (rank 16)** — page-at-a-time, no chrome; sequenced behind single-page (rank 8).
- [ ] **External annotations (rank 17)** — the §14-sanctioned formulation (stored externally, never modifying the document); three subprojects, and the storage decision (sidecar format/location) is Brandon's when this box opens.
- [ ] **Idle cursor hide + auto-scroll (rank 18)** — small reader-comfort pair.
- [x] **MuPDF manifest bump (rank 19)** — DONE 2026-09-15 (Brandon's go): the manifest bundles 1.28.2 matching dev/CI, with x-checker-data; see the build commit. The vir-gtk-capi adoption rider stays dependency-ordered on vir-gtk's capi lane (1.0.1 fallback per the recorded decision).
- [ ] **Test-harness leftovers (rank 20)** — bench JSON output + regression diff, `tests/lsan.supp` for the leak-sanitizer pass, the FwCache pretty-printer placeholder.
- [ ] **Split `fw-window.c` (rank 21; on explicit go)** — kept tracked (2026-09-15, Brandon): no reader value; the named candidates are the ~400-line `build_ui` constructor and the action handlers. See the Remaining box above.

### Final audit 2026-09-13 (THE FINAL AUDIT: NEW findings, one line each; full detail in audit-final/Framework/FINAL-REPORT.md)
- [x] **HIGH (NEW): PDF metadata stack out-of-bounds
  *(DONE 2026-09-15: one-line clamp of n to sizeof buf - 1 in metadata_lookup; ASan-verified on a crafted 1024-byte-title PDF; pinned by regress-final-audit /pdf-metadata-clamp + /pdf-metadata-roundtrip.)* (fw-document-pdf.c:1258-1262): metadata_lookup treats fz_lookup_metadata's return as a buffer length; MuPDF documents it as the UNTRUNCATED size, so a PDF with Title/Author/Keywords > 1023 bytes overruns the 1024-byte stack buffer via Document Properties.** One-line clamp of n to sizeof buf - 1.
- [x] **HIGH (CONFIRMED): DjVu ddjvu_document_get_* calls run unlocked
  *(DONE 2026-09-15: render_lock covers the fetch, the miniexp walk, and the pageinfo probes in all four queries; ddjvu_miniexp_release added on every path (the leak fixed in the same commit); the lock-discipline/miniexp-ownership header paragraph written. Pinned by regress-final-audit /djvu-locked-queries.)* against the shared context, racing render workers** (fw-document-djvu.c:409,496,521,549,565,597,640). Take render_lock around all of them; add the lock-discipline header paragraph.
- [x] **HIGH (CONFIRMED): MOBI INDX second-pass infinite loop on read_var_len length 0**
  *(DONE 2026-09-15: the CNCX vl.length == 0 guard ported; pinned by regress-final-audit /mobi-indx-zero-len with a hand-built AZW3 whose INDX claims 127 varlen bytes from the record's last byte; verified it hangs hot without the guard.)* (fw-mobi-parser.c:604-610; the CNCX loop has the guard, this pass does not).
- [ ] MED: Memory-safety family:
  *(DONE 2026-09-15: all seven, one commit; cache in-flight count, ExtractCtx owned refs + document capture, MOBI bare get_resources, EPUB/FB2 256 MB entry caps, MOBI </style guard, hdr_length in gsize, CBR volatile fz_try locals + the CRITICAL RULES header block. ASan+UBSan pass: 7/8 with the documented corpus-soak RSS exception.)* cache rendering-bool reset defeats the eviction guard (fw-cache.c:681 vs 796-808, replace with in-flight count); ExtractCtx raw window* across the portal async (fw-window.c:886-897; the "weak: not refed" comment manufactures a lifetime guarantee that does not exist); MOBI get_resources extra ref never dropped (transfer-none contract); EPUB/FB2 archive entries uncapped; MOBI inline <style> breakout missing the EPUB "</style" guard; MOBI hdr_length 32-bit overflow; CBR non-volatile fz_try locals.
- [ ] MED: Dead-code sweep:
  *(DONE 2026-09-15: all six removals approved via prompts and executed, one hygiene commit per interface: cache dead API, webview PositionRequest machinery, MOBI eager GdkTexture decode, cmp comparators, fw_document_close wrapper + slot, reflow close chain.)* fw_cache_get_page + fw_cache_page_ready (zero callers); webview PositionRequest machinery (~55 lines, duplicated by the live user-script path); MOBI eager GdkTexture decode (never read); fw_document_close wrapper + reflow close chain (both dead lifecycles); cmp_uint/cmp_guint32 duplicates.
- [ ] MED: Docs residue:
  *(DONE 2026-09-15: text/plain in the desktop MimeType; the DnD decision recorded in spec 5.3; section 2 renumbered sequentially with real state keys; fallback claim qualified for TXT/MD; all named CLAUDE/README/roadmap/tests-README stale bits fixed; the gschema descriptions moved to the WebKit vocabulary.)* desktop MimeType missing text/plain (TXT claimed by spec + shipped in the file dialog); spec drag-and-drop still "TBD" (decided in code v0.10); two sections numbered 2.4 + state-sketch keys zoom/scroll_y (code: zoom_level/scroll_position); fallback-to-MuPDF claim false for TXT/MD under fixed-pages mode; CLAUDE.md stale bits ("three stress tests" → seven, 6/7 ASan posture, Phase 6 open → shipped, get_resources "EPUB-only" → EPUB+MOBI, reflow gate missing Markdown, scope list missing TXT, Calibre-Library-as-default-corpus); roadmap corpus-root box + screenshots box + fw-window line count; README HiDPI shipped-claim + Phase 13.1 parenthetical + "5-figure" textbook; spec search debounce 300ms → GtkSearchEntry 150ms default; tests/README FW_TEST_CORPUS_ROOT is runtime, not configure-time.
- [ ] MED: Em-dash ban:
  *(DONE 2026-09-15: zero rendered em-dashes across roadmap, README (entities included), CLAUDE, spec, tests/README, foliate-rewrite, and the patchnotes title + newest entries; all listed slop kills applied except the AGPL sentence trio, deliberately kept verbatim-per-document for licensing clarity.)* 348 rendered violations (roadmap 152 bold-label separators, README 68 including mdash entities, CLAUDE.md 47, foliate-rewrite 47, spec 29); plus slop kills: "Best-in-Class UX" Phase-14 tagline, "Leveraging", "Seamless Auto-Reload", "marquee delivery", "design north star", "utilizing", "seamless" duplicated across README/roadmap, "core performance differentiator" duplicated spec/CLAUDE.
- [ ] MED: GitHub:
  *(DONE 2026-09-15: manifest bumped to MuPDF 1.28.2 + x-checker-data on both sources; CI hardened (SHA pin, concurrency, weekly schedule probe, stress harness compiles, regress targets run); release.yml publishes the tag annotation verbatim; dependabot (actions-only) + a bug template added; the no-force-push rulesets are active on main and v* tags per Brandon's go. Flathub: submission prep done (docs/flathub-submission.md), the submission itself rides the blitz release tag per decision.)* manifest bundles MuPDF 1.27.1 while dev/CI link 1.28.2 (bump + stress soak before the next Flatpak build); Flathub gate now open (type:git+tag verified); submission prep + x-checker-data on Brandon's go; no tag-triggered release automation; CI hardening (SHA pin, concurrency, schedule probe, compile the stress harness); optional no-force-push ruleset for the v1.0.0 anchor; issue template + actions-only dependabot.
- [x] LOW: Robustness:
  *(DONE 2026-09-15: coords zero-init, fz_try-wrapped handler registration at both sites, 64-level recursion caps on the three miniexp walkers, guint32 MOBI record index, CBR cross-thread read documented as deliberate.)* DjVu coords[4] uninitialized on short rect annotations; fz_register_document_handlers outside fz_try (pdf:331, cbr:588); miniexp walker recursion unbounded; MOBI guint16 truncation; CBR width/height cross-thread reads formally UB.
- [x] LOW: Comment truth batch (19 items, FINAL-REPORT L5):
  *(DONE 2026-09-15: all items corrected; O1 fw-window.c module header and O3 the no-source-globs comment landed in the same pass; O2 the CBR setjmp block rode the memory-safety commit; the fw-webview.h registry wording verified accurate as-is.)* window reflow-search fields that live in fw-webview, kinetic-scrolling ownership parenthetical, MOBI block-model ghosts, "FB2/TXT to come", residual FwReflowView refs, epub_encryption_is_drm cross-ref, cache header strategy/window numbers, "display lists", unreachable thumb_worker branch, CBR header threading paragraph, DjVu stale search comment pair, fw-state.h reflow_block dead field, fw-sandbox Phase-15 wording, zoom-storm header, fw-webview registry mechanism.
- [x] LOW: Housekeeping:
  *(DONE 2026-09-15: .claude/settings.local.json deleted and /.claude/ + /.cache/ ignored by the repo; both .gitignore tombstones pruned; compile_commands.json now a relative symlink with the recreate step documented in CLAUDE.md; i18n scaffold left as boilerplate per the audit.)* delete .claude/ + add /.claude/ and /.cache/ to repo .gitignore; relative compile_commands.json symlink + document the recreate step; prune framework-spec.md and claude.run .gitignore tombstones; i18n scaffold implies translatability that does not exist (localization = full string-marking pass).
- [ ] Feature candidates logged (FINAL-REPORT L4, ranked):
  *(SLOTTED 2026-09-15 as Phase 21 above, in rank order. The two gated items were ruled the same day: both §13 amendments granted (continue-reading recents and state.json bookmarks are viewer state); the Markdown highlighter rides a hand-rolled tokenizer, no new dependency; the fw-window split stays tracked on explicit go.)* reflow auto-reload parity (#1); PDF page labels; window geometry persistence; recolor light/dark keys; reading measure control; reflow print parity-or-disable; thumbnail sidebar; single-page mode; stext pre-warm; Markdown local images (syntax highlighting = the one dependency-flagged item); pinch-zoom; DjVu margin-crop; idle cursor hide. §13-GATED on Brandon's ruling: Open Recent, navigation bookmarks.

**CONFIRMED-prior (final-audit verification):** the entire Wave-4 BUGS set verified still open and unchanged (DjVu lock race, MOBI INDX loop, cache UAF, CBR volatile, miniexp leak, ExtractCtx, MOBI over-ref + three polish records); the 1.0.0 lane fixed none of them. SUPERSEDED (verified fixed): all 19 Wave-4 DOCS findings and the full Wave-4 GITHUB set (description/topics/homepage, Release, tag). Slop-reader verdict: genuinely human voice; the roadmap bullet-separator em-dash pattern is the one systemic vector.

## Post-blitz verification (2026-09-16)

*Findings from the post-blitz verification day (launch every app, exercise every surface against real data; audit-final/POST-BLITZ-VERIFICATION.md). Fixed-and-shipped items land as their own patch release per the repo cadence; the rest are recorded boxes.*

- [x] **Reflow position restore always restored to the top of the book** *(FIXED, v1.0.2, 2026-09-16: the reporter's anchor pick matched the injected non-rendered `<style id=fw-reading-css>` in head (rect.top 0, wins every pick) and restore prefers the anchor over scroll_y; the pick now requires a rendered box (`r.height > 0`). Live-verified on a real EPUB: scroll into chapter one, close, reopen lands in the chapter. Pinned by tests/stress/regress-webview-anchor.c.)* found by the verification day's live walk: state.json held a deep scroll_y with the poisoned anchor and every reopen opened the cover at 0%.
- [ ] Diagnostic honesty, polish class: a reflow open of a MISSING file reports "Unsupported file type: .txt" (the extension, from the MuPDF fallback factory) because the reflow refusal's real error (ENOENT) is discarded at the quiet fall-through in `fw_window_open_file`; the dialog misleads for missing-file cases. Recorded 2026-09-16, cosmetic.
