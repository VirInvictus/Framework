...
- `FwReflowDocument` (interface, `src/fw-reflow-document.h`) — the live vtable surface is `open`/`close`, `produce_html` (the hot path: format → stitched HTML + image table), `get_toc`, `get_metadata`. `fw_reflow_document_new_for_path` is the reflow factory; `fw_reflow_path_is_supported` is the gate the window checks. The old block-model slots and `FwBlock` structures have been completely stripped as of v0.77.
...
- `FwWebView` (`fw-webview.c`) — wraps `WebKitWebView`; loads the stitched HTML, registers the `framework-img://` scheme handler against the per-load image table, pushes reading style (`fw_webview_set_reading_style`) and search (`WebKitFindController`-backed) onto the page, and caches the reading position for state save. This is the reflow renderer that replaced `FwReflowView` in v0.76. `FwReflowSidebar` (`fw-reflow-sidebar.c`) is the reflow TOC sidebar and is **retained** — it is independent of the deleted block-model renderer.
