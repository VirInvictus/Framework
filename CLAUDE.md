...
- `FwReflowDocument` (interface, `src/fw-reflow-document.h`) — the live vtable surface is `open`/`close`, `produce_html` (the hot path: format → stitched HTML + image table), `get_toc`, `get_metadata`. `fw_reflow_document_new_for_path` is the reflow factory; `fw_reflow_path_is_supported` is the gate the window checks. **Dead-but-still-present (Phase 17.5b cleanup pending):** the block-model slots `get_block_model` / `get_image` / `find_block_by_anchor`, the `fw_reflow_document_search` wrapper + `FwReflowHit`, and the `FwBlock` GObject. Every backend still builds a block model that nothing renders now that `FwReflowView` is gone; the only remaining consumer is `tests/stress/stress-reflow.c`. 17.5b strips all of it (it entangles with `produce_html` in the EPUB/MOBI backends, where one parse loop feeds both).
