...
Framework is a native Linux (Hyprland-first, GNOME-compatible) multi-format document viewer (PDF, DjVu, CBZ, CB7, CBT, CBR, XPS, EPUB, FB2, MOBI, AZW3, TXT, Markdown) in C17, plain GTK4 under an owned Kanagawa Dragon stylesheet (libadwaita dropped in v0.80.0), Meson. It has **two render pipelines**: a velocity-driven pre-cache engine for fixed-layout formats (PDF/DjVu/comics/XPS), and a WebKitGTK reflow stack for ebook formats (EPUB/FB2/MOBI/AZW3/TXT/Markdown) that bypasses MuPDF's fixed layout. Project version lives in `meson.build`. `spec.md` is the authoritative design doc; `roadmap.md` tracks phase status; `patchnotes.md` is per-release notes.
...
Sanitizer builds use the `-Dsanitize=` array option (choices: `address`, `undefined`, `leak`, `thread`). `address` and `undefined` are the standard dev combo; the recorded posture is every test clean natively, and all but `stress-corpus-soak` under ASan+UBSan (its RSS cap trips on sanitizer memory overhead alone: `failures=0`), `liblsan` and `libtsan` are not installed on Brandon's Fedora and would need `sudo dnf install` first if you want them.
...
The registered suite's corpus is the repo's gitignored **`.testfiles/`** (canonical filenames in `tests/README.md`; `FW_TEST_CORPUS_ROOT` overrides at runtime). For ad-hoc debugging and benchmarking beyond those samples, Brandon's PDF/DjVu library at **`/home/bdkl/docs/Calibre Library/`** is the default source; it is read-only. Mind the spaces in the path (quote it). Example:
...
Pick samples that cover the failure modes you care about: large textbooks (font/JPEG2000-heavy, stresses the 8x context model from `fw-document-pdf.c` (see `MAX_RENDER_INSTANCES` and the comment block above it)), comics/scanned books (cheap to render but bytes-heavy under fit-width), `.djvu` files (single-mutex render path), poster-format PDFs (high-zoom memory pressure). For "find me a random sample," `find "/home/bdkl/docs/Calibre Library" -name '*.pdf' | shuf -n 10` is the pattern in use.
...
- **Open dispatch lives in `fw-window.c::fw_window_open_file`, not the factory.** For each path it first calls `fw_reflow_path_is_supported` (TXT/Markdown/FB2/EPUB/MOBI/PRC/AZW/AZW3); if true it tries `fw_window_open_reflow` (the WebKit reflow pipeline) and only on failure falls through to `fw_document_new_for_path` (MuPDF). So the *effective* default for ebook formats is reflow; the MuPDF fixed-layout path is the automatic fallback. The window holds either a `document` + `FwView` **or** a `reflow_doc` + `FwWebView`, swapped via a `GtkStack` with `"empty" | "document" | "webview"` pages (the old `"reflow"` block-model page went away with `FwReflowView` in v0.76).

**The Reflow Pipeline (`fw-reflow-*.c` + `fw-webview.c`)** is the second render path: ebook formats rendered in a `WebKitWebView`, not rasterized pages and not (as of v0.76) native block widgets. Each `FwReflowDocument` backend parses its format and emits a single stitched HTML document via `produce_html`; `FwWebView` loads it, with images served through the `framework-img://` URI scheme and typography/themes pushed live as CSS custom properties. See `docs/foliate-rewrite.md` for the original (now-superseded) block-model design and Phase 17 in `roadmap.md` for the WebKit pivot.

**Reference model (2026-07): the EPUB reader design reference is shifting from Foliate to Calibre's viewer** (the WebKit-engine EPUB approach that Phase 17 already borrowed from Calibre, per the `c05d3f9` commit). Calibre's viewer is the preferred model going forward for reader/presentation decisions: resource serving, publisher CSS, font handling, DRM/obfuscation semantics. The `.foliate-js/` parsers stay canonical for byte-format *parsing* (PalmDB/KF8, OPF/spine walking, FB2), and `.komikku/` for the comics pager; only the reader/presentation philosophy moves to Calibre. A fuller doc reframe (README "Influences", `docs/`) is deferred; this note records the direction.

- `FwReflowDocument` (interface, `src/fw-reflow-document.h`): the live vtable surface is `open`, `produce_html` (the hot path: format → stitched HTML + image table), `get_toc`, `get_metadata`, and `get_resources` (v0.79: path-keyed table backing `framework-img://<doc-id>/res/<path>` URIs: publisher CSS, de-obfuscated fonts; EPUB and MOBI/KF8 flow CSS ship it, the other formats return NULL). `fw_reflow_document_new_for_path` is the reflow factory; `fw_reflow_path_is_supported` is the gate the window checks. The old block-model slots and `FwBlock` structures have been completely stripped as of v0.77.
- Reflow backends: `FwReflowDocumentTxt` (`fw-reflow-document-txt.c`, v0.40), `FwReflowDocumentFb2` (`fw-reflow-document-fb2.c`, libxml2 walker since v0.57), `FwReflowDocumentEpub` (`fw-reflow-document-epub.c`, libarchive ZIP → OPF/spine → `htmlReadMemory` per chapter, NCX + EPUB3 nav.xhtml TOC, since v0.42/v0.56), `FwReflowDocumentMobi` (`fw-reflow-document-mobi.c`, KF7 + KF8/AZW3, v0.52/v0.55), and `FwReflowDocumentMd` (`fw-reflow-document-md.c`, Markdown, v0.75). The low-level PalmDB/PalmDOC/HuffDic/EXTH/KF8 byte-format parsing lives in `fw-mobi-parser.c` (a standalone parser, ported line-by-line from `.foliate-js/mobi.js`). **Markdown** is converted to HTML by **md4c** (MIT, vendored in `src/md4c/`, built as a separate static lib; GitHub dialect with `MD_FLAG_NOHTML` to block raw-HTML/script injection). All reflow formats go through the WebView via `produce_html` + the shared `fw-reflow-html` module (reading CSS, `find_body`, script-strip, img-rewrite).
- `FwWebView` (`fw-webview.c`): wraps `WebKitWebView`; loads the stitched HTML, registers the `framework-img://` scheme handler against the per-load image table plus (v0.79) the path-keyed resource table behind the `/res/<zip-path>` namespace (publisher CSS, fonts; extension-first MIME, `.js` refused), pushes reading style (`fw_webview_set_reading_style`) and publisher-style toggling (`fw_webview_set_publisher_styles`, flips DOM `.disabled` live) and search (`WebKitFindController`-backed) onto the page, and caches the reading position for state save. Carries the navigation policy (v0.79): fragment jumps pass, external links open via `GtkUriLauncher`, all other link-click navigation is blocked; a compiled WebKit content filter denies every remote (`http(s)`/`ws(s)`) subresource fetch. This is the reflow renderer that replaced `FwReflowView` in v0.76. `FwReflowSidebar` (`fw-reflow-sidebar.c`) is the reflow TOC sidebar and is **retained**; it is independent of the deleted block-model renderer.

**The velocity-driven cache (`fw-cache.c`) is the heart of the render path; read it before changing anything in it.** Three tiers and a state machine:
...
**Other modules:** `fw-application.c` (single-instance `GtkApplication`, file-open dispatch), `fw-theme.c` (owned `data/style.css` stylesheet provider + portal `color-scheme` reader driving dark/light; replaced `AdwStyleManager` in v0.80.0), `fw-window.c` (header bar, actions, keybindings, search-bar UI, navigation history with two `GArray<NavEntry>` stacks pushed only on explicit jumps; print operation; embedded-file extraction with sanitized output paths; Document Properties + Keyboard Shortcuts dialogs; an owned bottom-center `GtkRevealer` toast; `GFileMonitor`-backed auto-reload that saves state, re-opens the document, and toasts on every `CHANGES_DONE_HINT`; the reflow-vs-fixed open dispatch above; comic-layout toggles: manga/webtoon/facing-pages), `fw-sidebar.c` (`GtkListView` + `GtkTreeListModel` + `FwTocItem` GObject; `fw_sidebar_set_current_page` walks the underlying `FwTocItem` tree, expands ancestor `GtkTreeListRow`s, then selects the row in the flat model), `fw-search.c` (async find controller: runs the page-by-page scan on a worker thread, posts hits back via `g_idle_add_full`, emits `hits-changed` / `current-changed` / `search-finished` signals; the view subscribes to repaint highlights and reveal the active hit; detached-worker model since v0.65 so cancel never blocks the main loop), `fw-state.c` (per-document JSON state in `$XDG_DATA_HOME/framework/state.json`, LRU-pruned to 500 entries; stores page/scroll/zoom/rotation for fixed-layout and first-block index for reflow), `fw-sandbox.c` (Landlock LSM hardening: `fw_sandbox_drop_execute` drops filesystem EXECUTE + mknod rights at startup so a document-parser RCE can't escalate; no-op on old kernels; v0.38), `fw-fonts.c` (`fw_fonts_register` registers bundled font dirs with FontConfig so reflow text renders without depending on system fonts), `fw-comicinfo.c` (ComicInfo.xml metadata extraction for CBZ/CB7/CBT/CBR archives, feeding Document Properties; CBR parses at open, ZIP comics lazily at dialog time; v0.83.0), `fw-dialog-widgets.c` (owned dialog shell + boxed-list row helpers: `fw_dialog_new`/`fw_pref_group`/`fw_value_row`, extracted from `fw-window.c`; v0.83.0), `fw-debug.c` (the `FW_DEBUG` trace machinery).
...
Framework is strictly a viewer (spec §13). Don't add: annotations, export/save-as, recent-files UI, tabs, formats beyond the shipped set (PDF/DjVu/CBZ/CB7/CBT/CBR/XPS/EPUB/FB2/MOBI/AZW3/TXT/Markdown), vim bindings, modal interfaces. Phase status in `roadmap.md` is the source of truth for what's landed vs. deferred; check it before assuming a feature is missing on purpose.

## Reference repos (`.foliate/`, `.foliate-js/`, `.komikku/`)

Three vendored upstream sources kept locally for the Phase 13.1
reflow work (all six phases shipped through v0.67.0; see
...
| `.komikku/` | Python / GTK4 | GNOME manga/webtoon reader. Reference for the reader pager logic and chapter handling. (See `.komikku/komikku/reader/pager/`) |
...
| MuPDF (system dep) | AGPL-3.0 | **NO source copies** | We link the system library, which is fine; The shipping binary is effectively AGPL because of this link, even though Framework's source stays GPL-3-or-later. |
