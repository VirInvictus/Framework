...
- **`try_closest_rendered_page` zoom transition** (shipped in v0.28.0; see `src/fw-cache.c`) &mdash; each `CacheEntry` retains up to 3 prior-zoom snapshots; `fw_cache_get_texture` returns the closest match by zoom for GTK to auto-scale, so continuous Ctrl+scroll zoom shows a sharp scaled preview instead of a blurry one-zoom-back fallback. Pattern from sioyek `pdf_renderer.cpp:236`.
- **TTL+LRU hybrid eviction** (shipped in v0.26.0; see `src/fw-cache.c`) &mdash; `last_access_us` per `CacheEntry`, bumped on every cache hit; bytes-cap eviction sorts outside-priority candidates oldest-first. Pages the user just scrolled back to survive when the cap fires.
- **Per-instance MuPDF store size scaling** (shipped in v0.26.0; see `src/fw-document-pdf.c:pdf_pick_store_size`) &mdash; the eight per-render-instance `fz_context`s size their store by file size (16/32/64/128 MB), so novels don't pay textbook overhead and textbooks aren't pinched.
