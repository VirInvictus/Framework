...
* **A crafted PDF could smash the stack through Document Properties.** `metadata_lookup` treated MuPDF's `fz_lookup_metadata` return as a buffer length when the header documents it as the untruncated size, so a PDF whose Title/Author/Keywords exceeded 1023 bytes sent the trailing-whitespace trim loop reading and zero-writing past a 1024-byte stack buffer. The length is clamped; verified under ASan with a crafted file and pinned by regression tests (over-long values truncate to 1023 bytes; short values round-trip whole).
* **DjVu text/TOC/link queries raced the render threads.** The four query functions called `ddjvu_document_get_outline/pagetext/pageanno/pageinfo` with no render_lock while render workers were inside the library, and those calls block internally pumping the shared message queue; search runs on a worker thread and link lookups fire on mouse motion. The lock now covers fetch, walk, and release, mirroring the PDF backend's discipline (documented in the file header), and the never-released miniexp expressions are returned to the pool, closing the per-page retention.
* **A crafted MOBI/AZW3 could hang the parser forever.** The INDX second pass's value loop advanced neither position nor count when a variable-length integer read returned length 0, spinning and allocating without bound on hostile input; the guard the CNCX loop already had is ported over. The regression test builds a minimal AZW3 whose index claims 127 varlen bytes from its record's last byte, and it hangs hot without the fix.
* **The memory-safety family (seven fixes).** The cache's per-page rendering bool became an in-flight job count (a blanket reset at zoom time could let Tier-1 eviction free a parsed handle a worker was still rendering with); the attachment-extraction context now owns refs to the window and captures the document (the old raw pointer dangled if the window closed while the portal folder dialog was up, and a mid-dialog file switch would have used the wrong document); MOBI `get_resources` honors its transfer-none contract (the extra ref leaked the resource table and its flow CSS on every MOBI/AZW3 open); EPUB/FB2 archive entries are capped at the CBR backend's 256 MB before allocation; MOBI inline `<style>` text containing `</style` is refused like the EPUB collector's; the MOBI INDX header-length check computes in gsize (a 32-bit wrap fed the TAGX probe a wild pointer); and the CBR backend's `fz_try`-crossing locals are volatile, with the setjmp CRITICAL RULES block added to the file header. The whole family ran the suite under ASan+UBSan (7/8, with only the documented corpus-soak RSS-cap exception; 8/8 native).
* **Robustness LOWs.** DjVu link rects zero-initialize a short coordinate list; MuPDF handler registration moved inside `fz_try` at two sites that could abort the process on throw; the DjVu s-expression walkers bound their recursion at 64 levels (file-controlled depth could exhaust the stack); the MOBI text-record index is computed in 32 bits so a high KF8 boundary cannot alias an earlier record past the bounds check; and the CBR unlocked page-size reads are documented as a deliberate tolerance instead of looking like an oversight.

### Removed

* Six dead code surfaces, each approved before deletion: the unused public cache API (`fw_cache_get_page`/`fw_cache_page_ready`), the WebView's PositionRequest machinery (about 55 lines duplicated by the live user-script position path), the MOBI parser's eager GdkTexture decode (up to 4096 wasted decodes per book open; the WebView decodes the raw bytes itself), the duplicate array comparators in the MOBI backend, the `fw_document_close` wrapper and interface slot, and the reflow `close` chain across seven files.

### Packaging and CI

* **The Flatpak bundles MuPDF 1.28.2**, the same version dev machines and CI link, closing the version split. The sha256 comes from the official archive tarball, and `x-checker-data` on both manifest sources lets the Flathub bot propose bumps. Flathub submission prep lives in `docs/flathub-submission.md`; the submission itself rides this release tag, and the in-repo manifest sources v1.0.1 the same way it sourced v1.0.0.
* **CI hardened.** The checkout action is SHA-pinned, a concurrency group cancels superseded runs, and a weekly schedule probe surfaces `fedora:latest` drift between releases instead of at release time. The stress harness now compiles in CI, and the two synthetic regress targets run there; the corpus stays gitignored, and tests/README now states the real corpus-absent semantics the first CI run exposed.
* **Tags publish themselves.** A new release workflow creates the GitHub Release from the pushed tag's annotation, byte for byte, so the verbatim-tag rule has no paraphrase layer to drift. Dependabot (GitHub Actions only) and a minimal bug-report template round out the contributor surface, and two active rulesets block force-push and deletion on main and on `v*` tags (no review requirements; the v1.0.0 anchor is protected).

### Documentation

* Spec section 2 is numbered sequentially, both state sketches use the real saved keys, the drag-and-drop decision is recorded (v0.10: same open path, replaces the current document), the fallback-to-MuPDF claim is qualified (TXT and Markdown have no fixed-layout branch), and the mission list includes AZW3. The desktop file claims `text/plain`, matching the long-shipped file-dialog support. CLAUDE.md, README, tests/README, the roadmap boxes, and the GSettings descriptions now match shipped reality: eight suite targets and the sanitizer posture, the .testfiles corpus default, the reflow gate's formats, `get_resources` shipping on EPUB and MOBI, the 150 ms search debounce, the real icon tree, and the fractional-scaling qualifier.
* The em-dash ban is enforced across the rendered docs (348 instances recast, the 43 mdash entities included) with the audit's whole slop list killed: the Best-in-Class tagline, Leveraging, Seamless Auto-Reload, marquee delivery, design north star, utilizing, the throat-clear, the verbatim-duplicated sentences, and the stamped Komikku line. Nineteen stale code comments now describe the shipped design, fw-window.c has a real module header, and src/meson.build records the no-source-globs policy.

### Decisions recorded

* Spec §13 is amended by ruling (2026-09-15): continue-reading recents and navigation bookmarks are viewer state when sourced from and stored in Framework's own state.json; anything that manages files or writes into documents stays out of scope. The post-1.0 feature lane is slotted as roadmap Phase 21 with all 21 ranked boxes; the Markdown syntax-highlighting box is pinned to a hand-rolled tokenizer (no new dependency), and the `fw-window.c` split stays tracked on explicit go.

## v1.0.0 (2026-09-13)

*First tagged release: the 1.0.0 anchor for the whole 0.6.0 to 0.83.1 arc. Per the recorded tag policy, every version before this one ships untagged (a single exemption note in roadmap.md covers them all), and every release after this one gets its own tag. What follows is the release in full, for anyone arriving at this anchor cold.*

* **Two render pipelines.** Fixed-layout formats (PDF, DjVu, CBZ/CB7/CBT/CBR comics, XPS) render through the velocity-driven pre-cache engine: persistent thumbnails, a parsed-handle window, a bytes-aware surface cache (512 MB default, tunable), eight independent MuPDF contexts for parallel rendering, in-flight `fz_cookie` abort, and priority dispatch that reorders the render queue by what you are actually looking at. Fast scrubbing paints thumbnails instead of stuttering; zoom transitions keep the previous zoom's pixels until the sharp render lands. The design was proven by measurement (Phase 12's seven-target stress suite, clean under ASan/UBSan) rather than hope.
* **A real ebook path.** EPUB, MOBI, AZW3, FB2, TXT, and Markdown (v0.40 through v0.75) moved off MuPDF's fixed layout onto a WebKitGTK reflow pipeline: foliate-js-derived parsers, one stitched HTML document per book, reading typography and four themes pushed as CSS, publisher stylesheets with de-obfuscated embedded fonts (EPUB and KF8, v0.79/0.82) plus a dark-theme lightness transform so publisher CSS never glares, repaired TOC and cross-chapter links in every format, and a content filter plus HTML scrubbing that keeps books from reaching the network. Markdown rides the same path via the vendored md4c.
* **Hyprland-native without leaving GNOME (v0.80.0).** libadwaita dropped; Framework is plain GTK4 under an owned Kanagawa Dragon/Lotus stylesheet with portal-driven dark/light, a floating TOC that never squeezes the page, live fit-width/fit-page recompute on resize, tiled-window awareness, and an F12 hide-chrome mode.
* **The stability sweep (Phase 20, v0.83.0).** Fixed for real: a CBR cancellation bug that blanked the rest of a comic after one fast scroll, a search indicator that stuck on "Searching…", a color-scheme portal inversion that forced light mode, and a handful of leaks (a GVariant per theme query, the file dialog, MOBI failure paths), all pinned by the `regress-phase20` suite target. ComicInfo.xml metadata and a live reading-progress header for ebooks landed in the same pass.
* **Presentable pages, whatever the file.** Comic DPI normalization (v0.70), fit-width keyed to the typical body page instead of the widest (v0.69), CBR background page-size probing, hue-preserving invert colors (v0.22), reading ruler, loupe, margin cropping, smart text selection, auto-reload for LaTeX/Typst workflows, and printing through the system dialog.
* **Packaging that actually ships.** The Flatpak builds end-to-end against org.gnome.Platform 50 with bundled MuPDF and DjVuLibre; AppStream metadata carries four real screenshots and curated milestone notes; permissions stay tight (no network, documents through portals); Landlock drops filesystem execute rights at startup so a parser exploit cannot escalate.
* **This release also closes the pre-tag docs sweep:** the spec's GSettings table, primary menu, and shortcut tables now describe the shipped app; the ghost reading-two-column toggle (orphaned when the v0.76 renderer removed its consumer) is gone; format lists agree across README, metainfo, spec, and CLAUDE.md; and `.fb2.zip` joined the file dialog's filter to match long-shipped support.
